...
/*		if (HTTP_FW_UPDATE) {
			...
			os_printf("set online firmware update check interval to %ums\n", HTTP_FW_UPDATE_INTERVAL);
		}*/
	}
...
uint8 ICACHE_FLASH_ATTR cron_parser(char * cronline, bool onTimer) {
	...
	/*do {
		...
		del_pos = os_strstr(cronline, " ");
		if (del_pos) l = del_pos - cronline;
		else l = os_strlen(cronline);
		if (l >= 255) {
			os_printf("cron_parser: field is too long %d, must be < 255\n", l);
			os_free(val); os_free(val2); os_free(val3);
			return;
		}
		os_bzero(val, 255);
		os_strncpy(val, cronline, l);
		os_printf("val %s\n", val);
		switch (i) {
		case 0: //seconds
			do {
				del_pos2 = os_strstr(val, ",");
				if (del_pos2) l = del_pos2 - val;
				else l = os_strlen(val);
				if (l == 0) break;
				if (l >= 6) {
					os_printf("cron_parser: second range exceeds the max length of 5 chars\n");
					os_free(val); os_free(val2); os_free(val3);
					return;
				}
				os_bzero(val2, 6);
				os_strncpy(val2, val, l);
				os_printf("val2 %s\n", val2);
				range = false;
				r1 = 255;
				do {
					del_pos3 = os_strstr(val2, "-");
					if (del_pos3) {
						range = true;
						l = del_pos3 - val2;
					}
					else l = os_strlen(val2);
					if (l == 0) break;
					if (l >= 3) {
						os_printf("cron_parser: second field > 2 digits\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_bzero(val3, 3);
					os_strncpy(val3, val2, l);
					os_printf("val3 %s\n", val3);
					if (os_strncmp(val3, "*", 1) != 0)
						r = atoi(val3);
					else { range = true;  r = 0; r1 = 0; r2 = 59; }
					if (r < 0 || r > 59) {
						os_printf("cron_parser: seconds are from 0 to 59\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_printf("r %d, r1 %d, r2 %d\n", r, r1, r2);
					if (range) {
						if (r1 == 255) r1 = r;
						else {
							if (r2 != 59) r2 = r;
						...
						if (onTimer) secondsOn[r] = 1;
						else secondsOff[r] = 1;
					}
					if (del_pos3) val2 = del_pos3 + 1;
				} while (del_pos3);
				if (del_pos2) val = del_pos2 + 1;
			} while (del_pos2);
			break;
		case 1: //minutes
			//j = 0;
			do {
				...
				if (l == 0) break;
				if (l >= 6) {
					os_printf("cron_parser: minute range exceeds the max length of 5 chars\n");
					os_free(val); os_free(val2); os_free(val3);
					return;
				}
				os_bzero(val2, 6);
				...
				do {
					...
					if (l == 0) break;
					if (l >= 3) {
						os_printf("cron_parser: minute field > 2 digits\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					} 
					os_bzero(val3, 3);
					...
					if (r < 0 || r > 59) {
						os_printf("cron_parser: minutes are from 0 to 59\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_printf("r %d, r1 %d, r2 %d\n", r, r1, r2);
		...
		case 2: //hours
			...
			do {
				...
				if (l == 0) break;
				if (l >= 6) {
					os_printf("cron_parser: hour range exceeds the max length of 5 chars\n");
					os_free(val); os_free(val2); os_free(val3);
					return;
				}
				os_bzero(val2, 6);
				...
				do {
					...
					if (l == 0) break;
					if (l >= 3) {
						os_printf("cron_parser: hour field > 2 digits\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_bzero(val3, 3);
					...
					if (os_strncmp(val3, "*", 1) != 0)
						r = atoi(val3);
					else { range = true;  r = 0; r1 = 0; r2 = 23; }
					if (r < 0 || r > 23) {
						os_printf("cron_parser: hours are from 0 to 23\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_printf("r %d, r1 %d, r2 %d\n", r, r1, r2);
		...
		case 3: //days
			...
			do {
				...
				if (l == 0) break;
				if (l >= 6) {
					os_printf("cron_parser: day range exceeds the max length of 5 chars\n");
					os_free(val); os_free(val2); os_free(val3);
					return;
				}
				os_bzero(val2, 6);
				...
				do {
					...
					if (l == 0) break;
					if (l >= 3) {
						os_printf("cron_parser: day field > 2 digits\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_bzero(val3, 3);
					...
					if (os_strncmp(val3, "*", 1) != 0)
						r = atoi(val3);
					else { range = true;  r = 1;  r1 = 1; r2 = 31; }
					if (r < 1 || r > 31) {
						os_printf("cron_parser: days are from 1 to 31\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_printf("r %d, r1 %d, r2 %d\n", r, r1, r2);
					if (range) {
						if (r1 == 255) r1 = r;
							...
							if (r2 != 31) r2 = r;
							for (k = r1; k <= r2; k++) {
								if (onTimer) daysOn[k - 1] = 1;
								else daysOff[k - 1] = 1;
							}
						}
					}
					else {
						if (onTimer) daysOn[r - 1] = 1;
						else daysOff[r - 1] = 1;
					}
					if (del_pos3) val2 = del_pos3 + 1;
				} while (del_pos3);
		...
		case 4: //months
			...
			do {
				...
				if (l == 0) break;
				if (l >= 6) {
					os_printf("cron_parser: month range exceeds the max length of 5 chars\n");
					os_free(val); os_free(val2); os_free(val3);
					return;
				}
				os_bzero(val2, 6);
				...
				do {
					...
					if (l == 0) break;
					if (l >= 3) {
						os_printf("cron_parser: month field > 2 digits\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_bzero(val3, 3);
					...
					if (os_strncmp(val3, "*", 1) != 0)
						r = atoi(val3);
					else { range = true;  r = 1; r1 = 1; r2 = 12; }
					if (r < 1 || r > 12) {
						os_printf("cron_parser: months are from 1 to 12\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_printf("r %d, r1 %d, r2 %d\n", r, r1, r2);
					if (range) {
						if (r1 == 255) r1 = r;
							...
							if (r2 != 12) r2 = r;
							for (k = r1; k <= r2; k++) {
								if (onTimer) monthsOn[k - 1] = 1;
								else monthsOff[k - 1] = 1;
							}
						}
					}
					else {
						if (onTimer) monthsOn[r - 1] = 1;
						else monthsOff[r - 1] = 1;
					}
					if (del_pos3) val2 = del_pos3 + 1;
				} while (del_pos3);
				if (del_pos2) val = del_pos2 + 1;
		...
		case 5: //days-of-the-week
			...
			do {
				...
				if (l == 0) break;
				if (l >= 6) {
					os_printf("cron_parser: days-of-the-week range exceeds the max length of 5 chars\n");
					os_free(val); os_free(val2); os_free(val3);
					return;
				}
				os_bzero(val2, 6);
				...
				do {
					...
					if (l == 0) break;
					if (l >= 3) {
						os_printf("cron_parser: days-of-the-week field > 2 digits\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_bzero(val3, 3);
					...
					if (os_strncmp(val3, "*", 1) != 0)
						r = atoi(val3);
					else { range = true;  r = 0; r1 = 0; r2 = 6; }
					if (r < 0 || r > 6) {
						os_printf("cron_parser: days-of-the-week are from 0 to 6\n");
						os_free(val); os_free(val2); os_free(val3);
						return;
					}
					os_printf("r %d, r1 %d, r2 %d\n", r, r1, r2);
		...
		/*case 6: //timezone
			l = os_strlen(val);
			if (l > 3 || l < 1) {
				os_printf("cron_parser: timezone range is from -11 t to 13\n");
				os_free(val); os_free(val2); os_free(val3);
				return;
			}
			tz = atoi(val);
			...
			break;
		default:
			os_printf("too many fields in cronline \"<seconds> <minutes> <hours> <days> <months> <days-of-the-week>\" - ignored\n");
			os_free(val); os_free(val2); os_free(val3);
			return;
		}
		if (del_pos) cronline = del_pos + 1;
		i++;
	} while (del_pos);*/
	//os_free(val); os_free(val2); os_free(val3);
	return 0;
