...
SIGNAL=`grep "signal:" ${TMPFILE}|awk -F":" '{print $2}'|tr -d "\t"`
...
if [[ "${AUTHORIZED}" == "yes" && "${AUTHENTICATED}" == "yes" && "${ASSOCIATED}" == "yes" ]]; then
        echo "${DATETIME}: engine-room-fan station ${STATION_MAC} is connected for ${CONNECTED_TIME}, signal ${SIGNAL}, rx ${RX_BYTES} bytes, tx ${TX_BYTES} bytes" | nc -un -w 1 ${LOGGING_DAEMON_IP} ${LOGGING_DAEMON_PORT}
else
