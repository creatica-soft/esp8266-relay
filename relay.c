...
#define USER_AGENT "esp8266-relay"
#define HTTP_TIMEOUT 3 //sec

//update relay parameters from http://WEB_SERVER:WEB_SERVER_PORT/RELAY_PATH
#define RELAY_HTTP_UPDATE true
#define RELAY_HTTP_UPDATE_INTERVAL 300000 //check for new timer parameters every ms
#define RELAY_HTTP_UPDATE_TASK_PRIORITY 5 //should be lower than 8
#define RELAY_PATH "/relay-parameters.txt"
/*RELAY_PATH file
RELAY_TASK_INTERVAL=<value>
TIMEZONE=<value>
CRONLINE_ON=<value>
CRONLINE_OFF=<value>
DATA_LOW_THRESHOLD=<value>
DATA_HIGH_THRESHOLD=<value>
LOW_ON=<true|false>
DATA_TASK_INTERVAL=<value>
*/
...
//increase relay version every time you modify this file to avoid re-downloading the same image over and over;
//for example, make it /relay-2.bin but upload the binary to the web server with the previously flashed version, i.e. /relay-1.bin
#define FW_PATH "/relay-1.bin"

#define TIME_DRIVEN true
#define TIMER_ON_CRONLINE "* 0 9-17  * * *" //sec min hour day month day-of-week
#define TIMER_OFF_CRONLINE "* 0 0-8,18-23 * * *" //sec min hour day month day-of-week
...
#define DATA_DRIVEN true
#define DATA_LOW_THRESHOLD "17.0" //for example, temperature in deg C
#define DATA_HIGH_THRESHOLD "18.0"
#define LOW_ON "true" //turn on the heater, for example, when temperature falls below DATA_LOW_THRESHOLD
                    // and turn it off, when it rises above DATA_HIGH_THRESHOLD
                    // "false" should be for humidity metric - ventilation will be on when the metric is above DATA_HIGH_THRESHOLD
                    // and off, when below DATA_LOW_THRESHOLD
#define DATA_PATH "/env_lastupdate_t.php" //for temperature or "/env_lastupdate_h.php" for humidity - see https://github.com/creatica-soft/bme280
#define DATA_TASK_INTERVAL 60000 //ms
...
#define ESP_CONFIG_TASK_SS 4000
#define RELAY_HTTP_UPDATE_TASK_SS 4200
#define FW_UPDATE_TASK_SS 4000
#define RELAY_TASK_SS 1400
#define AP_CHECK_TASK_SS 1000
#define DATA_TASK_SS 4000
#define HIGH_WATER_MARK_WARNING 50
#define HIGH_WATER_MARK_CRITICAL 10

static unsigned int relay_task_interval, data_task_interval;
static char * tz = TIMEZONE;
static int secondsOn[60], minutesOn[60], hoursOn[24], daysOn[31], monthsOn[12], dweekOn[7];
static int secondsOff[60], minutesOff[60], hoursOff[24], daysOff[31], monthsOff[12], dweekOff[7];
static float metric, data_low_threshold, data_high_threshold;
static bool low_on;
...
static int parse_body(char * body) {
	...
	char * timezone = NULL;
	char * low_threshold = NULL;
	char * high_threshold = NULL;
	char * l_on = NULL;
	char * data_task_interval_char = NULL;
	int e = 0;
	char * s, * ss, * sss;
	...
	char * bcopy = (char *)malloc(body_len);
	if (!bcopy) {
		ESP_LOGE(TAG, "parse_body: malloc - %s", strerror(errno));
		return 14;
	}

	const char * parameters[8] = { "RELAY_TASK_INTERVAL", "TIMEZONE", "CRONLINE_ON", "CRONLINE_OFF", "DATA_LOW_THRESHOLD", "DATA_HIGH_THRESHOLD", "LOW_ON", "DATA_TASK_INTERVAL" };
	for (int i = 0; i < 8; i++) {
		strncpy(bcopy, body, body_len);
		s = strstr(bcopy, parameters[i]);
		if (!s) {
			ESP_LOGE(TAG, "parse_body: unable to find %s", parameters[i]);
			return 1;
		}
		ss = strtok(s, "\r\n");
		if (!ss) {
			ESP_LOGE(TAG, "parse_body: unable to find new line and/or carriage return");
			return 2;
		}
		sss = strtok(ss, "=");
		if (!sss) {
			ESP_LOGE(TAG, "parse_body: unable to find equal sign (=)");
			return 3;
		}
		switch (i) {
		case 0:
			relay_task_interval_char = strtok(NULL, "/r/n");
		...
		case 4:
			if (DATA_DRIVEN) {
				low_threshold = strtok(NULL, "/r/n");
				if (!low_threshold) {
					ESP_LOGE(TAG, "parse_body: unable to get low_threshold");
					return 9;
				}
				//ESP_LOGI(TAG, "parse_body: low_threshold %s", low_threshold);
				if (isdigit((unsigned char)low_threshold[0])) {
					data_low_threshold = atof(low_threshold);
					//ESP_LOGI(TAG, "parse_body: data_low_threshold %.1f", data_low_threshold);
				}
			}
			break;
		case 5:
			if (DATA_DRIVEN) {
				high_threshold = strtok(NULL, "/r/n");
				if (!high_threshold) {
					ESP_LOGE(TAG, "parse_body: unable to get high_threshold");
					return 10;
				}
				//ESP_LOGI(TAG, "parse_body: low_threshold %s", low_threshold);
				if (isdigit((unsigned char)high_threshold[0])) {
					data_high_threshold = atof(high_threshold);
					//ESP_LOGI(TAG, "parse_body: data_high_threshold %.1f", data_high_threshold);
				}
			}
			break;
		case 6:
			if (DATA_DRIVEN) {
				l_on = strtok(NULL, "/r/n");
				if (!l_on) {
					ESP_LOGE(TAG, "parse_body: unable to get low_on");
					return 11;
				}
				//ESP_LOGI(TAG, "parse_body: l_on %s", l_on);
				if (strcasecmp(l_on, "true") == 0)
					low_on = true;
				else low_on = false;
			}
			break;
		case 7:
			if (DATA_DRIVEN) {
				data_task_interval_char = strtok(NULL, "/r/n");
				if (!data_task_interval_char) {
					ESP_LOGE(TAG, "parse_body: unable to get data_task_interval_char");
					return 12;
				}
				//ESP_LOGI(TAG,"parse_body: relay_task_interval_char %s", relay_task_interval_char);
				if (isdigit((unsigned char)data_task_interval_char[0])) {
					data_task_interval = atoi(data_task_interval_char);
					//ESP_LOGI(TAG, "parse_body: data_task_interval %u", data_task_interval);
				}
				else {
					ESP_LOGE(TAG, "parse_body: data_task_interval is not a number");
					return 13;
				}
			}
			break;
...
static void http_update_task(void *arg)
{
	...
	while (1) {
		ESP_LOGI(TAG, "http_update_task: checking for an updated relay config...");
		b = buf;
		t = sizeof(buf);
		r = 0;
		s = -1;
		...
		char buff[1024];
		int l = sprintf(buff, "%s %s %s\r\nHost: %s:%s\r\nConnection: close\r\nUser-Agent: %s\r\nAccept: %s\r\n\r\n", HTTP_METHOD, RELAY_PATH, HTTP_VERSION, WEB_SERVER, WEB_SERVER_PORT, USER_AGENT, "text/plain");
	...
	sleep:
		...
		vTaskDelay(pdMS_TO_TICKS(RELAY_HTTP_UPDATE_INTERVAL));
	}
}
...
static void data_task(void *arg)
{
	...
	while (1) {
		ESP_LOGI(TAG, "data_task: getting metric...");
		b = buf;
		t = sizeof(buf);
		r = 0;
		s = -1;
		...
		int err = getaddrinfo(WEB_SERVER, WEB_SERVER_PORT, &hints, &res);
		if (err != 0) {
			ESP_LOGE(TAG, "data_task: DNS lookup failed - %d", err);
			ESP_LOGW(TAG, "data_task: out of precausion turning relay off...");
			gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
			vTaskDelay(pdMS_TO_TICKS(rand));
			continue;
		}
		...
		s = socket(res->ai_family, res->ai_socktype, 0);
		if (s < 0) {
			ESP_LOGE(TAG, "data_task: socket failed - %s", strerror(errno));
			goto sleep;
		}
		//ESP_LOGI(TAG, "request: allocated socket");

		if (connect(s, res->ai_addr, res->ai_addrlen) != 0) {
			ESP_LOGE(TAG, "data_task: connect failed - %s", strerror(errno));
			goto sleep;
		}
		//ESP_LOGI(TAG, "request: connected");
		...
		receiving_timeout.tv_usec = 0;
		if (setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, &receiving_timeout, sizeof(receiving_timeout)) < 0) {
			ESP_LOGE(TAG, "data_task: failed to set socket receiving timeout");
			goto sleep;
		}
		//ESP_LOGI(TAG, "request: set socket receiving timeout success");

		char buff[1024];
		int l = sprintf(buff, "%s %s %s\r\nHost: %s:%s\r\nConnection: close\r\nUser-Agent: %s\r\nAccept: %s\r\n\r\n", HTTP_METHOD, DATA_PATH, HTTP_VERSION, WEB_SERVER, WEB_SERVER_PORT, USER_AGENT, "text/plain");
		//ESP_LOGI(TAG, "request: sending GET request:\n%s", buff);
		if (write(s, buff, l) < 0) {
			ESP_LOGE(TAG, "data_task: write failed - %s", strerror(errno));
			goto sleep;
		}
		//ESP_LOGI(TAG, "request: socket send success");
		...
		do {
			...
			t = buf + sizeof(buf) - b;
			if (t <= 0) {
				ESP_LOGE(TAG, "data_task: receive buffer is full");
				ESP_LOGW(TAG, "data_task: out of precausion turning relay off...");
				gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
				goto sleep;
			}
		...
		if (strncmp(buf, "HTTP/1.", 7) == 0) {
			...
			if (http_status == 200) {
				...
				body_len = strlen(body);
				if (body_len != 0) {
					//ESP_LOGI(TAG, "data_task: body is %s", body);
					if (isdigit((unsigned int)body[0])) {
						metric = atof(body);
						//ESP_LOGI(TAG, "data_task: metric is %.1f", metric);
					}
					else {
						ESP_LOGE(TAG, "data_task: metric is not a number");
						ESP_LOGW(TAG, "data_task: out of precausion turning relay off...");
						gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
						goto sleep;
					}
				...
				{
					ESP_LOGE(TAG, "data_task: body_len = 0");
					ESP_LOGW(TAG, "data_task: out of precausion turning relay off...");
					gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
					goto sleep;
				}
				...
				ESP_LOGE(TAG, "data_task: http_status %d", http_status);
				ESP_LOGW(TAG, "data_task: out of precausion turning relay off...");
				gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
				goto sleep;
			}
			...
			ESP_LOGE(TAG, "data_task: not an HTTP packet");
			ESP_LOGW(TAG, "data_task: out of precausion turning relay off...");
			gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
			goto sleep;
		}
	...
	sleep:
		...
		UBaseType_t hwm = uxTaskGetStackHighWaterMark(NULL);
		if (hwm <= HIGH_WATER_MARK_CRITICAL)
			ESP_LOGE(TAG, "data_task: uxTaskGetStackHighWaterMark %lu", hwm);
		else if (hwm <= HIGH_WATER_MARK_WARNING)
			ESP_LOGW(TAG, "data_task: uxTaskGetStackHighWaterMark %lu", hwm);

		vTaskDelay(pdMS_TO_TICKS(data_task_interval));
	}
}
...
static void relay_task(void *arg) {
	...
	char * hms[3];
	bool time_on, time_off, data_on, data_off, time_keep, data_keep;
	int i, j;
	time_t now;
	struct tm timeinfo;
	...
	while (1) {
		...
		if (TIME_DRIVEN) {
			if (timeinfo.tm_year >= 120) { //we got the time
				...
				if (time_on) ESP_LOGI(TAG, "time matches crontabOn schedule");
				if (time_off) ESP_LOGI(TAG, "time matches crontabOff schedule");
			}
			else {
		...
		if (DATA_DRIVEN) {
			if (LOW_ON) { //heating-like temperature metric
				if (metric > data_low_threshold) {
					data_on = false;
					ESP_LOGI(TAG, "metric %.1f above data_low_threshold %.1f, on is false", metric, data_low_threshold);
				}
				if (metric < data_high_threshold) {
					data_off = false;
					ESP_LOGI(TAG, "metric %.1f below data_high_threshold %.1f, off is false", metric, data_high_threshold);
				}
				if (metric > data_low_threshold && metric < data_high_threshold) {
					data_keep = true;
					ESP_LOGI(TAG, "metric %.1f is between data_low_threshold %.1f and data_high_threshold %.1f", metric, data_low_threshold, data_high_threshold);
				}
				else data_keep = false;
			}
			else {//for ventilation-like humidity metric
				if (metric < data_low_threshold) {
					data_on = false;
					ESP_LOGI(TAG, "metric %.1f below data_low_threshold %.1f, on is false", metric, data_low_threshold);
				}
				if (metric > data_high_threshold) {
					data_off = false;
					ESP_LOGI(TAG, "metric %.1f above data_high_threshold %.1f, off is false", metric, data_high_threshold);
				}
				if (metric > data_low_threshold && metric < data_high_threshold) {
					data_keep = true;
					ESP_LOGI(TAG, "metric %.1f is between data_low_threshold %.1f and data_high_threshold %.1f", metric, data_low_threshold, data_high_threshold);
		...
		if (time_keep && data_keep) goto sleep;
		else if (time_on && data_on) {
			ESP_LOGI(TAG, "relay_task: turning relay on...");
			gpio_set_level(GPIO_NUM_0, 0); //Set GPIO0 as low - level output.
		}
		else if (time_off || data_off) {
			ESP_LOGI(TAG, "relay_task: turning relay off...");
			gpio_set_level(GPIO_NUM_0, 1); //Set GPIO0 as high - level output.
		}
		else {
			ESP_LOGI(TAG, "relay_task: the current time does not match any of the cronline or other conditions are not met");
		}
...
static void esp_config_task(void *arg)
{
	...
	while (1) {
		...
		if (notificationValue) {
			...
			char par[1024];
			sprintf(par, "RELAY_TASK_INTERVAL=%u\nTIMEZONE=%s\nCRONLINE_ON=%s\nCRONLINE_OFF=%s\nDATA_LOW_THRESHOLD=%s\nDATA_HIGH_THRESHOLD=%s\nLOW_ON=%s\nDATA_TASK_INTERVAL=%u\n", RELAY_TASK_INTERVAL, TIMEZONE, TIMER_ON_CRONLINE, TIMER_OFF_CRONLINE, DATA_LOW_THRESHOLD, DATA_HIGH_THRESHOLD, LOW_ON, DATA_TASK_INTERVAL);
			parse_body(par);
			if (RELAY_HTTP_UPDATE) {
				if (pdPASS != xTaskCreate(&http_update_task, "http_update_task", RELAY_HTTP_UPDATE_TASK_SS, NULL, RELAY_HTTP_UPDATE_TASK_PRIORITY, NULL))
			...
			if (DATA_DRIVEN)
				if (pdPASS != xTaskCreate(&data_task, "data_task", DATA_TASK_SS, NULL, DATA_TASK_PRIORITY, NULL))
					ESP_LOGE(TAG, "esp_config_task: failed to create data task");
